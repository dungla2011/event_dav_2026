...
// Lưu ý: các hằng DEF_BAOKIM_* / DEF_MOMO_* được define trong AppServiceProvider::register()
// (common.php nạp qua composer autoload TRƯỚC khi .env load, nên không thể dùng env() ở đây).
...
function nowy_vn($time = null, $sepe = "-")
{
    return $datetime = date("d".$sepe."m".$sepe."Y", $time);
}
...
//Password: (redacted)
//Merchan ID : 35589
//Api Key: (xem .env BAOKIM_API_KEY)
//Secret Key: (xem .env BAOKIM_API_SECRET)
...
class BaoKimAPI2021
{
    ...
    /* Bao Kim API key */
    const API_KEY = DEF_BAOKIM_API_KEY;
    const API_SECRET = DEF_BAOKIM_API_SECRET;
...
class clsMomo {
    ...
    //DEV
    //$partnerCode = (dev, redacted)
    //$accessKey = (dev, redacted)
    //$serectkey = (dev, redacted)
    //$endpoint = "https://test-payment.momo.vn/gw_payment/transactionProcessor";

    //PROD
    static public $partnerCode = DEF_MOMO_PARTNER_CODE; //4s
    static public $accessKey = DEF_MOMO_ACCESS_KEY;
    static public $serectkey = DEF_MOMO_SECRET_KEY;
