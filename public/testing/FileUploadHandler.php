<?php
// === TẠM KHÓA 2026-09-14 (lý do bảo mật: upload không xác thực, không lọc đuôi file -> RCE) ===
// Bỏ dòng die bên dưới để khôi phục hoạt động.
http_response_code(403);
header('Content-Type: application/json; charset=utf-8');
die(json_encode(['status' => 'error', 'message' => 'Service temporarily disabled']));
// === HẾT KHÓA ===
/**
...
function handleMultipartUpload() {
    ...
    // Check if file exists and generate version number
    if (file_exists($file_path)) {
        $filename = generateVersionedFilename($filename, $USER_FILES_DIR);
        $file_path = $USER_FILES_DIR . '/' . $filename;
        error_log("  → File exists, renamed to: {$filename}");
    }
...
function generateVersionedFilename($filename, $directory) {
    ...
    $version = 1;
    $new_filename = $name . '.' . $version . $extension;
    
    // Tăng version cho đến khi tìm được tên file chưa tồn tại
