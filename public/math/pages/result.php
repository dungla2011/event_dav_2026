...
<div class="container">
    ...
    <div class="actions">
        <button class="btn-action btn-primary" onclick="window.location.href='<?= url('?act=dashboard') ?>'">
            🏠 Về Trang Chủ
        </button>
...
<script>
    ...
    async function loadResults() {
        try {
            const response = await fetch(`<?= url('api/submission_detail.php') ?>?id=${submissionId}`, {
            ...
            const answered = data.answers.filter(a => a.isAnswered).length;
            const wrong = data.answers.filter(a => a.isAnswered && !a.isCorrect).length;
            ...
            document.getElementById('scoreCard').innerHTML = `
                <div class="${scoreClass}">
                    ...
                    <div class="score-details">
                        ...
                        ${notAnswered > 0 ? `
                        <div class="detail-item">
                            <div class="detail-label">⏸️ Chưa Làm</div>
                            <div class="detail-value" style="color: #999;">${notAnswered}</div>
                        </div>
                        ` : ''}
                    </div>
                </div>
                ...
                return `
                    <div class="answer-item ${statusClass}">
                        ...
                        <div class="answer-details">
                            <span>Đáp án của bạn: <strong class="${a.isCorrect ? 'answer-correct' : (a.isAnswered ? 'answer-wrong' : '')}">${a.userAnswer !== null ? a.userAnswer : 'Không trả lời'}</strong></span>
                            ${a.isAnswered && !a.isCorrect ? `<span> <strong class="answer-correct"></strong></span>` : ''}
    ...
    function retryExercise() {
        if (currentExerciseId) {
            window.location.href = `<?= url() ?>?act=quiz&exerciseId=${currentExerciseId}`;
        }
    }
