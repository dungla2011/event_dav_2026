...
@section('title', 'Edit Record - MongoDB CRUD')
                    ...
                    <div class="card">
                        <div class="card-header">
                            ...
                            <div class="card-tools">
                                <a href="{{ route('mongocrud.demo01.show', $record->_id) }}" class="btn btn-info btn-sm">
                            ...
                            <form id="editForm">
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label>Phone</label>
                                            <input type="text" class="form-control" name="phone" value="{{ $record->phone }}" maxlength="20">
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label>Age</label>
                                            <input type="number" class="form-control" name="age" value="{{ $record->age }}" min="0" max="150">
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="form-group">
                                    <label>Address</label>
                                    <textarea class="form-control" name="address" rows="2" maxlength="500">{{ $record->address }}</textarea>
                                </div>
                                
                                <div class="form-group">
                                    <label>Description</label>
                                    <textarea class="form-control" name="description" rows="3" maxlength="1000">{{ $record->description }}</textarea>
                                </div>
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" name="tags" 
                                                   value="{{ $record->tags ? implode(', ', $record->tags) : '' }}"
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control" name="metadata" rows="4" 
                                              placeholder='{"department": "IT", "position": "Developer"}'>{{ $record->metadata ? json_encode($record->metadata, JSON_PRETTY_PRINT) : '' }}</textarea>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-4 text-center">
                                        <a href="{{ route('mongocrud.demo01.show', $record->_id) }}" class="btn btn-info">
                                            <i class="fas fa-eye mr-1"></i>
                                            View Record
                                        </a>
                                    </div>
...
document.getElementById('editForm').addEventListener('submit', async function(e) {
    ...
    try {
        const response = await MongoCrudAPI.demo01.update(recordId, data);
        
        if (response.success) {
            MongoCrudUtils.showAlert('Record updated successfully!');
            setTimeout(() => {
                window.location.href = '{{ route("mongocrud.demo01.show", $record->_id) }}';
            }, 1000);
        } else {
        ...
        MongoCrudUtils.showAlert('Error updating record', 'error');
    } finally {
